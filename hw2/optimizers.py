...
class VanillaSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            # self.reg * p - is the derivation of the regulation function
            dp += self.reg * p
...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.v = {}
        # save the velocity in the p index of each param
        for i in range(len(params)):
            self.v[params[i][0]] = torch.zeros_like(params[i][0])
        # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        for i in range(len(self.params)):
            self.r[params[i][0]] = torch.zeros_like(params[i][0])
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg*dp
            self.r[p] = self.decay*self.r[p]+(1-self.decay)*torch.pow(dp, 2)
            p -= (self.learn_rate / torch.sqrt(self.r[p]+self.eps)) * dp
            # ========================
