...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            test_curr_epc_avg_loss = torch.sum(
                torch.tensor(test_res.losses)).item()/len(test_res.losses)
            ...
            if minimal_batches_count == 0:
                ...
                test_last_epc_avg_loss = torch.sum(
                    torch.tensor(test_loss[:-minimal_batches_count])).item()/minimal_batches_count
            # updating the epochs_without_improvement for early stopping
            if test_last_epc_avg_loss <= test_curr_epc_avg_loss and len(test_loss) > 0:
                epochs_without_improvement += 1
            else:
                epochs_without_improvement = 0

            # update the arrays:
            train_loss.extend([train_loss_t.item()
                               for train_loss_t in train_res.losses])
            test_loss.extend([test_loss_t.item()
                              for test_loss_t in test_res.losses])
