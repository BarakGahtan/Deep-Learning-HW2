...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(torch.zeros(
            size=(out_features, in_features)), wstd)
        self.b = torch.normal(torch.zeros(size=(out_features,)), wstd)
        # ========================
...
class ReLU(Block):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        # out =
        out = torch.where(x < 0, torch.zeros_like(x), x)
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        out = torch.zeros_like(dout)
        dx = torch.where(x < 0, out, dout)
        # ========================
...
class Sigmoid(Block):
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        dx = dout*self.grad_cache['out']*(1-self.grad_cache['out'])
        # ========================
...
class CrossEntropyLoss(Block):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        right_lable_score = x[range(N), y]
        tempLoss = torch.log(torch.sum(torch.exp(x), dim=1))-right_lable_score
        loss = torch.sum(tempLoss)/N
        # ========================
...
class Dropout(Block):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            ...
            rand_mat = torch.rand_like(x)
            mask = torch.where(rand_mat > self.p, ones_mask, zero_mask)
            out = x*mask
            out *= 1/(1-self.p)
            self.grad_cache["mask"] = mask
        else:
            out = x*(1-self.p)
            self.grad_cache["mask"] = ones_mask
        # ========================
...
class Sequential(Block):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        reverseList = list(self.blocks)
        reverseList.reverse()
        # print(type(reverseList))
        din = dout
        for block in reverseList:
...
class MLP(Block):
    ...
    def __init__(self, in_features, num_classes, hidden_features=(),
        ...
        preview_layer = in_features
        for hidden_layer in hidden_features:
            blocks.append(Linear(preview_layer, hidden_layer, **kw))
            if activation == 'relu':
                blocks.append(ReLU())
                if dropout > 0:
            ...
            preview_layer = hidden_layer
        blocks.append(Linear(preview_layer, num_classes, **kw))
