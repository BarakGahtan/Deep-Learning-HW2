...
def run_experiment(run_name, out_dir='./results', seed=None, device=None,
    ...
    fit_res = None
    # ====== YOUR CODE: ======
    prev_layer, _ = ds_train[0]
    input_size = prev_layer.shape
    filters_list = []
    for current_filter in filters_per_layer:
        for _ in range(layers_per_block):
            filters_list.append(current_filter)
    model = model_cls(input_size, 10, filters_list,
                      pool_every, hidden_dims).to(device)
    # print(model)
